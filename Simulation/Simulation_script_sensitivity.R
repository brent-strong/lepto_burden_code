...
n_h_values <- 0:3
mean_prob_values <- c(0.25)

# Function: scale k so that Beta(8.5k, 1.5k) matches target SD
beta_scale_from_sd <- function(target_sd) {
  
  if (target_sd <= 0) {
    stop("target_sd must be positive")
  }
  
  var_target <- target_sd^2
  
  k <- (12.75 / (100 * var_target) - 1) / 10
  
  if (k <= 0) {
    warning("No valid solution: target_sd too large for this parameterization")
    return(NA)
  }
...
# Sensitivity scale parameters
sens_spec <- c(
  1,
  beta_scale_from_sd(0.08),
  beta_scale_from_sd(0.04),
  beta_scale_from_sd(0.01)
)

# Corresponding SD values (must align with sens_spec order)
sd_lookup <- tibble(
  sens_spec = sens_spec,
  SD = c(0.11, 0.08, 0.04,0.01)
)
...
param_grid <- expand.grid(
  ...
  as_tibble() %>%
  mutate(n_jobs = x) %>%
  left_join(sd_lookup, by = "sens_spec") %>%
  relocate(SD, .after = n_jobs) %>%
  as.data.frame()
...
mean_log_rate <- mean(log(1/param_grid[j,2]*region_level_data$lepto_per_100k_2023))
...
sim_results <- mclapply(1:param_grid[j,5], function(i) {
  set.seed(param_grid[j,4]*1000 + i)  # optional, for reproducibility
  data_gen(
    beta_0 = mean_log_rate,
    beta_1 = log(1.05),
    mean_prob = param_grid[j,2],
    sigma_Phi = 0.5,
    sigma_phi = 0.5,
    sigma_tau=0.5,
    zeta = 0.06,
    multiplier_lower = 4,
    multiplier_upper = 8,
    sensitivity = c(0.85, 0.85),
    specificity = c(0.85, 0.85),
...
rate_PR_2024 <- sum(d_y_forecast[1:S] + y_2024[1:S])/sum(pop[1:S])
  ...
  zeta <- 0.06
  nu[1] ~ dbeta(cert*8.5,cert*1.5)
  nu[2] ~ dbeta(cert*8.5,cert*1.5)
  kappa[1] ~ dbeta(cert*8.5,cert*1.5)
  kappa[2] ~ dbeta(cert*8.5,cert*1.5)
  pi_A ~ dbeta(5,15)
})
...
model_output <- vector("list", length = param_grid[j, 5])

for(i in 1:param_grid[j, 5]){
  
  sim_results[[i]]$constant_list$mu_alpha_0 <- mu_alpha_0_val
  sim_results[[i]]$constant_list$cert <- param_grid[j,3]
  
  Rmodel <- nimbleModel(
    m_code,
    constants = sim_results[[i]]$constant_list,
  ...
  seeds_prelim <- rep(param_grid[j,4] * 1500,n_chains)
...
output_dir <- simulation_path("Simulation_sensitivity")
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

saveRDS(model_output,file=file.path(output_dir,
                            paste0("model_output_nh_",param_grid[j,1],"_SD_sens_spec_",param_grid[j,6],"_job_",
                            param_grid[j,4],".rds")))
